...
from .utils_2d import make_layers, get_padding, L2_dist
from .reslib_2d import ResNeXtBottleNeck, BasicResBlock
...
class spk_vq_vae_resnet(nn.Module):
	def __init__(self, param):
		...
		self.res2 = nn.Sequential(
			nn.Conv2d(conv1_ch, conv2_ch, conv2_ker, groups=1, padding=get_padding(conv2_ker), bias=False),
			nn.BatchNorm2d(conv2_ch),
			nn.Dropout2d(p=dropRate)
		)

		# resnet decoder
		self.deres2 = make_layers(conv2_ch, conv1_ch, conv2_ker, n_layers=1, decode=True, dropRate=0)
		self.deres1 = BasicResBlock(conv1_ch, conv1_ker, n_layers=2, decode=True, dropRate=dropRate)
		self.deres0 = nn.ConvTranspose2d(conv1_ch, org_dim, conv0_ker, padding=get_padding(conv0_ker))

		self.ds = nn.MaxPool2d(2)
	...
	def forward(self, x):
		...
		# reshape
		sz = h.size()
		h = h.view(sz[0], sz[1], -1)
		Z = h.view(sz[0]*sz[1], -1)
		...
		# inverse reshape
		h = W_j.view(-1, sz[2], sz[3])
		h = h.view(sz[0], sz[1], sz[2], sz[3])
